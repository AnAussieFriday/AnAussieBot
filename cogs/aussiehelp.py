    ...
    async def send_bot_help(self, mapping):
        ...
        for cog, commands in mapping.items():
            ...
            if commands:
                ...
                if cog_name != "Hidden" and cog_name != "Error" and cog_name != "No Category":
...
class AussieHelpCommand(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

        help_command = AnAussieHelpCommand()
        help_command.cog = self
        bot.help_command = help_command


async def setup(bot):
    await bot.add_cog(AussieHelpCommand(bot))
